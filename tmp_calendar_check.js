    ...
    (function () {
        ...
        document.addEventListener("click", (event) => {
            const target = event.target;
            if (!(target instanceof Element)) return;
            const copyTrigger = target.closest("[data-copy-training]");
            if (copyTrigger instanceof HTMLButtonElement) {
                const card = copyTrigger.closest(".calendar-day");
                if (!(card instanceof HTMLElement)) return;
                const trainingId = Number(card.getAttribute("data-training-id") || 0);
                if (!trainingId) return;
                const training = trainingsStore.find((item) => Number(item.id) === trainingId);
                if (!training) return;
                const statusNode = card.querySelector("[data-copy-status]");
                const showCopyStatus = (message, isError = false) => {
                    if (!(statusNode instanceof HTMLElement)) return;
                    statusNode.textContent = message;
                    statusNode.hidden = false;
                    statusNode.classList.toggle("calendar-event__copy-status--error", Boolean(isError));
                    const previousTimer = Number(statusNode.dataset.hideTimerId || 0);
                    if (previousTimer) {
                    ...
                    statusNode.dataset.hideTimerId = String(timerId);
                };
            ...
            if (sections.length) {
                ...
                sections.forEach((section) => {
                    ...
                    const normalizedItems = rawItems
                        .map((item) => normalizeLineBreaks(item).split("\n").map((part) => String(part || "").trim()))
                        .flat()
                        .filter(Boolean);
